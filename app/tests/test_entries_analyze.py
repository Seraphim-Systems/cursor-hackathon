...
async def test_post_analyze_skips_locked_summary_refreshes_unlocked(analyze_only_app) -> None:
    ...
    entry.insights = InsightsEmbedded.model_validate(_empty_insights())
    ...
    fake_user = MagicMock()
    fake_user.id = PydanticObjectId(owner_sub)
    analyze_only_app.dependency_overrides[get_current_user] = lambda: fake_user
